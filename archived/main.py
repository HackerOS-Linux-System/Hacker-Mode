...
from tkinter import font as tkfont, ttk, scrolledtext, messagebox
...
class HackerMode:
    ...
    def setup_language(self):
        ...
        self.translations = {
            'en': {
                ...
                'shortcut_note': "Configure Win + E in Sway to close HyperPlay, GameHub, Sober, or Lutris and reopen Hacker Mode",
            ...
            'pl': {
                ...
                'shortcut_note': "Skonfiguruj Win + E w Sway, aby zamykać HyperPlay, GameHub, Sober lub Lutris i otwierać Tryb Hakera",
    ...
    def setup_app_launchers(self):
        ...
        apps = [
            ("steam.png", self.get_text('steam'), ["steam", "-gamepadui"], False, True),
            ("heroic.png", self.get_text('heroic'), ["flatpak", "run", "com.heroicgameslauncher.hgl"], False, False),
            ("sober.png", self.get_text('sober'), ["flatpak", "run", "com.sober.app"], False, True),
            ("lutris.png", self.get_text('lutris'), ["lutris"], False, False),
            ("hyperplay.png", self.get_text('hyperplay'), ["flatpak", "run", "io.playhyper.hyperplay"], False, False),
            ("gamehub.png", self.get_text('gamehub'), ["flatpak", "run", "com.github.tkashkin.gamehub"], False, False)
        ]
        ...
        if os.path.exists(settings_path):
            try:
                ...
                settings_button = tk.Button(
                    menu_frame, image=self.settings_photo, command=self.show_settings_gui,
                ...
                settings_button = tk.Button(
                    menu_frame, text=self.get_text('settings'), command=self.show_settings_gui,
    ...
    def show_settings_gui(self):
        logging.debug("Showing settings GUI")
        try:
            ...
            style.configure("TNotebook.Tab", background=self.menu_color, foreground=self.text_color, font=self.button_font)
            style.map("TNotebook.Tab", background=[("selected", self.highlight_color)])

            notebook = ttk.Notebook(settings_window)
            notebook.pack(pady=10, padx=10, fill='both', expand=True)

            audio_frame = tk.Frame(notebook, bg=self.bg_color)
            notebook.add(audio_frame, text=self.get_text('audio'))
            tk.Button(audio_frame, text=self.get_text('increase_volume'), command=self.increase_volume, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(audio_frame, text=self.get_text('decrease_volume'), command=self.decrease_volume, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(audio_frame, text=self.get_text('toggle_mute'), command=self.toggle_mute, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Label(audio_frame, text=self.get_text('select_output'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            for output in self.get_audio_outputs():
                tk.Button(audio_frame, text=output, command=lambda o=output: self.set_audio_output(o), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=2)

            display_frame = tk.Frame(notebook, bg=self.bg_color)
            notebook.add(display_frame, text=self.get_text('display'))
            tk.Button(display_frame, text=self.get_text('increase_brightness'), command=self.increase_brightness, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(display_frame, text=self.get_text('decrease_brightness'), command=self.decrease_brightness, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(display_frame, text=self.get_text('toggle_theme'), command=self.toggle_theme, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Label(display_frame, text=self.get_text('select_resolution'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            for res in self.get_resolutions():
                tk.Button(display_frame, text=res, command=lambda r=res: self.set_resolution(r), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=2)

            network_frame = tk.Frame(notebook, bg=self.bg_color)
            notebook.add(network_frame, text=self.get_text('network'))
            tk.Button(network_frame, text=self.get_text('wifi_settings'), command=self.show_wifi_settings, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(network_frame, text=self.get_text('connect_wifi'), command=self.show_wifi_networks, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(network_frame, text=self.get_text('toggle_wifi'), command=self.toggle_wifi, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(network_frame, text=self.get_text('bluetooth'), command=self.show_bluetooth, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)

            power_frame = tk.Frame(notebook, bg=self.bg_color)
            notebook.add(power_frame, text=self.get_text('power'))
            tk.Button(power_frame, text=self.get_text('power_saving'), command=lambda: self.set_power_profile("power-saver"), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(power_frame, text=self.get_text('balanced'), command=lambda: self.set_power_profile("balanced"), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(power_frame, text=self.get_text('performance'), command=lambda: self.set_power_profile("performance"), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Label(power_frame, text=self.get_text('screen_timeout'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            self.timeout_var = tk.StringVar(value="5")
            tk.Entry(power_frame, textvariable=self.timeout_var, bg=self.menu_color, fg=self.text_color).pack(pady=5)

            general_frame = tk.Frame(notebook, bg=self.bg_color)
            notebook.add(general_frame, text=self.get_text('general'))
            tk.Button(general_frame, text=self.get_text('toggle_notifications'), command=self.toggle_notifications, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Button(general_frame, text=self.get_text('update_system'), command=self.show_update_system, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            tk.Label(general_frame, text=self.get_text('select_language'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            for lang in self.translations.keys():
                tk.Button(general_frame, text=lang, command=lambda l=lang: self.change_language(l), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=2)
            tk.Button(general_frame, text=self.get_text('shortcut_info'), command=self.show_shortcut_info, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)

            button_frame = tk.Frame(settings_window, bg=self.bg_color)
            button_frame.pack(pady=10)
            tk.Button(button_frame, text=self.get_text('apply'), command=lambda: self.apply_settings(settings_window), bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(side='left', padx=5)
            tk.Button(button_frame, text=self.get_text('close'), command=settings_window.destroy, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(side='left', padx=5)
        except Exception as e:
            logging.error(f"Error showing settings GUI: {e}")
    ...
    def apply_settings(self, window):
        ...
        try:
            ...
            if not timeout.isdigit() or int(timeout) <= 0:
                tk.Label(window, text=self.get_text('invalid_timeout'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=5)
                return
    ...
    def show_update_system(self):
        ...
        update_window.title(self.get_text('update_system'))
        update_window.geometry("600x400")
        update_window.configure(bg=self.bg_color)
        update_window.transient(self.root)
        update_window.grab_set()
        update_window.attributes('-type', 'dialog')
        update_window.attributes('-topmost', True)

        script_path = "/usr/share/HackerOS/Scripts/Bin/update_hacker-mode.sh"
        if not os.path.exists(script_path):
            tk.Label(update_window, text=self.get_text('update_script_missing'), bg=self.bg_color, fg=self.text_color, font=self.button_font).pack(pady=10)
            tk.Button(update_window, text=self.get_text('close'), command=update_window.destroy, bg=self.menu_color, fg=self.text_color, font=self.button_font).pack(pady=5)
            logging.error(f"Update script not found at {script_path}")
            return

        output_text = scrolledtext.ScrolledText(update_window, height=15, bg=self.menu_color, fg=self.text_color, font=self.button_font)
        ...
        try:
            ...
            if result.returncode == 0:
                output_text.insert(tk.END, self.get_text('update_output', output=result.stdout))
            else:
                output_text.insert(tk.END, self.get_text('update_failed', error=result.stderr))
            logging.debug(f"Update script output: {result.stdout} {result.stderr}")
        except Exception as e:
            output_text.insert(tk.END, self.get_text('update_failed', error=str(e)))
    ...
    def check_internet(self):
        ...
        try:
            # Primary check: nmcli connectivity
            ...
            if status == "full":
                return True
            # Fallback: Ping Google's DNS server
            ping_result = subprocess.run(["ping", "-c", "1", "8.8.8.8"], capture_output=True, text=True, check=False)
            logging.debug(f"Ping result: {ping_result.returncode}, output: {ping_result.stdout} {ping_result.stderr}")
    ...
    async def focus_app(self, app_id):
        ...
        try:
            await asyncio.sleep(1)  # Wait for the application to start
    ...
    def check_app_installed(self, command, app_name):
        ...
        try:
            if "flatpak" in command:
                ...
                return flatpak_id in installed_apps
            else:
    ...
    def launch_app(self, command, use_root=False, app_name="App", requires_internet=False):
        ...
        try:
            ...
            if not self.check_app_installed(command, app_name):
                ...
                messagebox.showerror(self.get_text('title'), self.get_text('app_not_installed', app=app_name))
                return
            ...
            app_id = command[-1] if "flatpak" in command else command[0].split("/")[-1]
    ...
    def restart_apps(self):
        ...
        for app in ["steam", "heroic", "sober", "lutris", "hyperplay", "gamehub"]:
