...
async function setFullscreen(appName, titleName, retries = 3, delay = 3000) {
    for (let i = 0; i < retries; i++) {
        try {
            // Use xdotool to send F11 key to the window with the given name
            await execPromise(`xdotool search --name "${titleName}" key F11`);
            log(`Set fullscreen for ${appName} using xdotool`, 'info');
            return true;
        } catch (err) {
...
ipcMain.handle('launchApp', async (event, appName) => {
    ...
    setTimeout(async () => {
        // Attempt to set fullscreen using xdotool
        await setFullscreen(appName, app.titleName);
    }, 3000);
    proc.on('close', () => {
...
ipcMain.handle('systemAction', async (event, action) => {
    const actions = {
        switchToPlasma: () => {
            ...
            exec('startplasma-wayland', (err) => {
                if (err) log(`Error launching startplasma-wayland: ${err}`, 'error');
            });
